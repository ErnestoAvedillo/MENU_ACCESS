...
def calculadora_torsion(request):
    ...
    if request.method == 'POST':
        try:
            ...
            material_obj = Material(nombre_material=datos_entrada_muelle['material'])
            muelle = MuelleTorsion(material=material_obj, wire_diameter=float(request.POST.get('diametero_hilo', 0)))
            ...
            print(f"Resultado del cálculo de muelle de torsión: {resultado}")            
        except Exception as e:
...
def get_data_spring(request):
    ...
    datos_muelle = {
        ...
        'diametro_hilo': to_float('diametero_hilo'),
